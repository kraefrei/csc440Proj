...
%     end
%     
%     t_tmpX = cell(size(tab,1),1);
%     for i = 1:size(tab,1)
%         t_tmpX{i,1} = num2str(tab{i,2});
%     end
%     
%     tab = [tab(:,1) tab(:,3:end) tab(:,2)];
%     
...
norm_sale = (out - mean(num_att_trim.SalePrice)) ./std(num_att_trim.SalePrice);
refined_mat = [table2array(str_att_num) tmp(:,2:end)];
...
scatter3(twodtrans(:,1),twodtrans(:,2),num_att_trim.SalePrice)
...
%notlog
mean_saleprice = mean(num_att_trim.SalePrice);
std_saleprice  = std(num_att_trim.SalePrice);
...
figure
grtrth = num_att_trim.SalePrice(1201:end);
